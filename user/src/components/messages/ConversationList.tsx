...
import {
  ...
  getConversationDetail, receiveMessageSuccess, resetConversationState, updateLastMessage, updateTotalConversationNotification
...
interface IProps {
  ...
  updateTotalConversationNotification: Function;
  conversation: {
    list: {
      requesting: boolean;
      error: any;
...
class ConversationList extends PureComponent<IProps> {
  ...
  onMessage = (message: { conversationId: string | number; text: any }) => {
    if (!message) {
      return;
    }
    const {
      ...
      updateLastMessage: dispatchUpdateLastMessage,
      updateTotalConversationNotification: dispatchUpdateTotalConversationNotification
    } = this.props;
    ...
    dispatchUpdateLastMessage({ conversationId: message.conversationId, message: message.text });
    if (activeConversation._id !== message.conversationId) dispatchUpdateTotalConversationNotification(message.conversationId);
  };
    ...
    return (
      <div className="conversation-list" onScroll={this.handleScroll.bind(this)}>
        ...
        <div className="c-list-item">
          ...
          <p className="text-center">No conversation was found.</p>
          )}
        </div>
      </div>
    );
