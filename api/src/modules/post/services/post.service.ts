...
export class PostService {
  ...
  public async create(
    ...
    if (image) data.imageId = image._id;
  ...
  public async update(
    ...
    const oldFileToDelete = image ? post.imageId : null;
    if (image) post.imageId = image._id;
    await post.save();
    await this.fileService.remove(oldFileToDelete);
  ...
  public async delete(id: string | ObjectId, user?: UserDto): Promise<boolean> {
    ...
    if (post.imageId) await this.fileService.remove(post.imageId);
  ...
  public async adminGetDetails(id: string): Promise<PostDto> {
    ...
    const image = post.imageId ? await this.fileService.findById(post.imageId) : null;
    const dto = new PostDto(post);
    if (image) dto.setImage(image);
    dto.meta = meta;
    return dto;
  }
