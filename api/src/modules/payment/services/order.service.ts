...
export class OrderService {
  constructor(
    ...
    // private readonly prformerService: PerformerService,
  ...
  public async search(req: OrderSearchPayload) {
    ...
    if (req.buyerId) query.buyerId = toObjectId(req.buyerId);
    if (req.userId) query.buyerId = toObjectId(req.userId);
  ...
  public async orderDetailsSearch(req: OrderSearchPayload) {
    ...
    const userIds = orders.map((o) => o.buyerId);
    // if (performerIds.length) {
    //   const performers = await this.userService.findByIds(performerIds);
    //   sellers.push(
    //     ...performers.map((p) => (new UserDto(p)).toResponse())
    //   );
    // }
    if (userIds.length) {
      const users = await this.userService.findByIds(userIds);
  ...
  public async getOrderDetails(id: string | ObjectId) {
    ...
    if (details.sellerId) {
      const seller = await this.userService.findById(details.sellerId);
      dto.seller = (new UserDto(seller)).toResponse();
    }
