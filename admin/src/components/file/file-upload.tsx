...
function beforeUpload(file) {
  ...
  const isLt2M = file.size / 1024 / 1024 < (config.NEXT_PUBLIC_MAX_SIZE_FILE || 20);
  if (!isLt2M) {
    message.error(`File must smaller than ${config.NEXT_PUBLIC_MAX_SIZE_FILE || 20}MB!`);
  }
  return isLt2M;
}
  ...
  render() {
    ...
    return (
      <Upload
        ...
        headers={headers}
      >
        {fileUrl && <span>Click to download</span>}
        {uploadButton}
      </Upload>
    );
  }
